...
def get_the_packet_and_agent_data(required_data,dates,project_id,center_id,filter_type):
    ...
    packet_or_agent_error_data, packet_or_agent_config,packet_or_agent_error_sum = {}, {}, {}
    ...
    for data in required_data:
        for date_values, factor in zip(dates_lists,values):
            ...
            if packet_or_agent_error_data.has_key(data):
                packet_or_agent_error_data[data].append(round((factor*error_value), 2))
            else:
                packet_or_agent_error_data[data] = [round((factor*error_value), 2)]

    for key,value in packet_or_agent_error_data.iteritems():
        packet_or_agent_error_sum[key] = sum(value)

    sorted_data                                 = (sorted(packet_or_agent_error_sum.items(), key=itemgetter(1), reverse=True))[:config_value]
...
def packet_agent_audit_random(request):
    ...
    audit_dict, random_dict, common_dict = OrderedDict(), OrderedDict(), OrderedDict()
    ...
    for i in xrange(len(production_records)):
        ...
        if (calculated_value <= audited_value):
