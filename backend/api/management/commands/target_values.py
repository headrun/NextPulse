        ...
        for pro_cen in proje_cent:
            ...
            for month_name,month_dates in months_dict.iteritems():
                ...
                for pac in tar_packs:
                    ...
                    import pdb;pdb.set_trace()
                    #final_bill_ppl.append(sum(bill_values[0]))
                    #fin_bill_ppl = sum(final_bill_ppl)
                    if bill_values:
                        billab_ppl = sum(bill_values[0])
                        fin_bill_ppl = final_bill_ppl.append(billab_ppl)
                    if fin_tar_val:
                    ...
                    if billable_hc['billable_agents__sum'] != None:
                        ...
                        productivity_val = float(sum(final_actual_val))/float(fin_bill_ppl)
