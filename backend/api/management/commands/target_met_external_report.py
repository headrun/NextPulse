...
class Command(BaseCommand):
    ...
    args = '[command]'
    help = 'generating target met report for External accuracy'
    ...
    def handle(self, *args, **options):
        ...
        from api.models import Project,RawTable,TeamLead,Targets, Externalerrors
        ...
        for project in projects_list:
            ...
            for tl in tls:
                ...
                audited_errors = Externalerrors.objects.filter(project=project.id, date=last_date).aggregate(Sum('audited_errors'))
                total_errors = Externalerrors.objects.filter(project=project.id, date=last_date).aggregate(Sum('total_errors'))
                ...
                if target_value > accuracy:
                    msg = str(project) + ' deos not met the given external accuracy target'
                    ...
                    to = [tl_email]
                    msg = EmailMessage("NextPulse External Accuracy Status", mail_body, 'nextpulse@nextwealth.in', to)
