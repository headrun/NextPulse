    ...
    def handle(self, *args, **options):
        ...
        for month_name,month_dates in months_dict.iteritems():
            ...
            proje_cent = ['Probe','NTT DATA Services TP','NTT DATA Services Coding','Federal Bank','Ujjivan','Gooru','Walmart Salem',\
                          ...
                          'IBM India and Sri Lanka','IBM NA and EU','IBM Arabia','IBM Latin America','IBM Sri Lanka P2P','Jumio']
            #proje_cent = ['Probe']
            for pro_cen in proje_cent:
                ...
                for date in dates_list:
                    max_date = RawTable.objects.filter(project=prj_id,center=center_id,date=date).aggregate(Sum('per_day'))
                    if max_date['per_day__sum'] != None:
                        date_check.append(date)
                #import pdb;pdb.set_trace()
                if len(date_check) > 0:
                    tars = Targets.objects.filter(\
                                   project=prj_id, center=center_id,from_date__gte=date_check[0],to_date__lte=date_check[-1]) 
                    tars_1 = Targets.objects.filter(\
                                   project=prj_id, center=center_id,from_date__lte=date_check[0],to_date__gte=date_check[-1])
                    if tars and prj_id != 1:
                    ...
                    for pac in tar_packs:
                        ...
                        if tar_values != [] and act_values != []:
                            ...
                            pac_tar_vals, pac_act_vals, pac_days, pac_sin_tar, prod_fin_cal, pac_prod_util, pac_bill_util = 0 , 0 , 0 , 0 , 0, 0, 0
                        if bill_values:
