...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for nw_manager in nw_managers:
            ...
            projects_list = Project.objects.filter(is_voice=False, display_project=True, is_weekly_mail=True).order_by('name').values_list('id', flat=True).distinct()
        ...
        for cm_manager in center_managers:
            ...
            projects_list = Project.objects.filter(center=center, display_project=True, is_voice=False, is_weekly_mail=True).order_by('name').values_list('id', flat=True).distinct()
            projects_list = list(projects_list)
            projects_list.sort(key=ordering_list.get)            
            if projects_list:
