...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        customers = Customer.objects.filter(name__is_active=True, project__is_daily_mail=True, \
            project__is_voice=False, project__display_project=True, \
                daily_mail=True, daily_mail_metrics=False).distinct()
        tls = TeamLead.objects.filter(name__is_active=True, project__is_voice=False, \
            project__display_project=True, project__is_daily_mail=True, \
                daily_mail=True).distinct()
        customer_metric_daily = Customer.objects.filter(name__is_active=True, daily_mail_metrics=True,\
             project__is_daily_mail=True, project__is_voice=False, \
                 project__display_project=True, daily_mail=False).distinct()
        custom_list = [30, 112, 160, 129, 159, 117, 180, 181, 182, 123, 113, 162, 114, 126, 119, 115, 118, 130, 127,
                       154, 158, 121, 156, 161, 116, 132]
        
        
        for customer in customers:
            customer_details = Customer.objects.filter(id=customer.id, project__is_daily_mail=True, \
                daily_mail=True, daily_mail_metrics=False).values_list('project', flat=True).distinct()                        
            if len(customer_details) > 0:
                send_metric_mail(customer, customer_details)
        
        for customer in customer_metric_daily:
            customer_details = Customer.objects.filter(id=customer.id, project__is_daily_mail=True,\
                 daily_mail_metrics=True, daily_mail=False).values_list('project', flat=True).distinct()
            if len(customer_details) > 0:
                send_mail_data(customer_details, customer, 'customer')                

        for tl in tls:
            tl_details = TeamLead.objects.filter(id=tl.id,project__is_daily_mail=True, daily_mail=True).\
        ...
        for nw_manager in nw_managers:
            ...
            projects_list = Project.objects.filter(is_voice=False ,display_project=True, is_daily_mail=True).\
        ...
        for cm_manager in center_managers:
            ...
            projects = Project.objects.filter(center=center, display_project=True, is_voice=False, is_daily_mail=True).\
                order_by('name').values_list('id', flat=True).distinct()
