...
def get_annotations(request):
    ...
    annotations = Annotation.objects.filter(key__contains='<##>' + chart_name + '<##>' + series_name + '<##>')
...
def add_annotation(request):
    ...
    prj_obj = Project.objects.filter(name='IBM')
...
def update_annotation(request):
    ...
    if action == "delete":
        anno = Annotation.objects.filter(epoch = epoch, created_by = request.user, key__contains = to_query)
        if anno:
            anno = anno[0]
            anno.delete()
