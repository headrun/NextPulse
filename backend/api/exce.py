...
def nw_exce(request):
    ...
    if main_data_dict['dwm_dict'].has_key('day') and main_data_dict['type'] == 'day':
        for sing_list in main_dates_list:
            total_done_value = RawTable.objects.filter(project=prj_id, center=center, date__range=[sing_list[0], sing_list[-1]])\
            ...
            main_data_dict['work_packet'], main_data_dict['sub_project'], main_data_dict['sub_packet'],main_data_dict['pro_cen_mapping'])
        ...
        for month_na,month_va in zip(\
            main_data_dict['dwm_dict']['month']['month_names'],main_data_dict['dwm_dict']['month']['month_dates']):
            month_name = month_na
...
def overall_exce(request):
    ...
    if main_data_dict['dwm_dict'].has_key('day') and main_data_dict['type'] == 'day':
        ...
        final_dict['overall_exception_details'] = graph_data_alignment_color(\
                                                    overall_exception_details,'data',level_structure_key,prj_id,center,'')
        final_dict['date'] = new_date_list
    elif main_data_dict['dwm_dict'].has_key('week') and main_data_dict['type'] == 'week':
        for sing_list in main_dates_list:
            data_date.append(sing_list[0] + ' to ' + sing_list[-1])
        ...
        final_dict['overall_exception_details'] = graph_data_alignment_color(\
                                                  final_overall_exception,'data', level_structure_key, prj_id, center,'')
        final_dict['date'] = data_date
    else:
        for month_na,month_va in zip(\
                main_data_dict['dwm_dict']['month']['month_names'],main_data_dict['dwm_dict']['month']['month_dates']):
    ...
    if main_data_dict['dwm_dict'].has_key('day') and main_data_dict['type'] == 'day':
        for sing_list in main_dates_list:
            total_done_value = RawTable.objects.filter(\
                                project=prj_id, center=center, date__range=[sing_list[0], sing_list[-1]])\
                                .values('date').annotate(total=Sum('per_day'))
            values = OrderedDict(zip(map(lambda p: str(p['date']), total_done_value), map(lambda p: str(p['total']), total_done_value)))
            for date_va, total_val in values.iteritems():
                if total_val > 0:
            ...
            level_structure_key = get_level_structure_key(\
            main_data_dict['work_packet'], main_data_dict['sub_project'], main_data_dict['sub_packet'],main_data_dict['pro_cen_mapping'])
            pre_scan_exception_details = pre_scan_exception_data(\
            sing_list, main_data_dict['pro_cen_mapping'][0][0],main_data_dict['pro_cen_mapping'][1][0])
